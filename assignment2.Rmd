...
Egoalt8 <- read_tsv("C:/Users/F/Documents/Politics and International relations/Data Analysis in Social Science III/UKDA-6614-tab/tab/ukhls_w8/h_egoalt.tab")
...
Hh8 <- read_tsv("C:/Users/F/Documents/Politics and International relations/Data Analysis in Social Science III/UKDA-6614-tab/tab/ukhls_w8/h_hhresp.tab")
```
...
JoinedEthn <- JoinedEthn %>%
        ...
        rename(alterRace_binary = binary_race)
```
...
TableRace <- JoinedEthn %>%
  ...
  filter(!is.na(alterRace_binary)) %>%
        count(egoRace, alterRace_binary)
...
# joining the individual-level (JoinedEthn) with the
# household-level data (Hh8)
Hh8 <- Hh8 %>%
        left_join(JoinedEthn, by = c("h_hidp"))
# removing all the results except for the White British,
# Indian and Pakistani ones
Hh8 <- Hh8 %>%
  mutate(new_race = recode(egoRacel_dv,
                           '1' = "White British",
                           '9' = "Indian",
                           '10' = "Pakistani"))
# checking the mutation is what I wanted it to be
table(Hh8$new_race)
# calculating mean and median children per household
# based on race
Hh8 %>%
  group_by(new_race) %>%
  summarise(mean(h_nkids_dv, na.rm = TRUE),
            median(h_nkids_dv, na.rm = TRUE))

# interpretation of results:
# this data shows that Pakistani families have on average
# the most children out of our sample of Indian, White 
# British and Pakistani families. Both in terms of the mean
# and the median 
# One thing that may have affected my findings is that
# the sample is too small to produce an effective analysis
# there's 18844 White British families and only 1603
# Indian and 2095 Pakistani families. So perhaps the
# dataset wasn't large enough to produce results reflective
# of the whole British population.
# However it is also possible that this dataset does reflect
# broad trends in British birthrates. Perhaps Pakistani
# families are often more recent immigrants to the UK
# as other research suggests that first generation immigrants
# are likely to have much larger families than second or 
# third generation immigrants. 
# (http://www.lse.ac.uk/News/Latest-news-from-LSE/2018/08-Aug# -2018/Many-children-of-immigrants-to-the-UK-have-smaller-fa# milies-than-their-parents)
# Hence the Indian families may be more likely to have 
# lived in the UK for more than one generation and thus
# be more like White British people in the size of their
# families.
